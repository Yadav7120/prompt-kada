...
async function startServer() {
  ...
  // GET profile — returns profile or defaults if not set yet
  app.get('/api/profile', authenticateToken, async (req: any, res) => {
    try {
      ...
      if (rows.length === 0) {
        ...
        return res.json({ display_name: 'Admin', email: '', theme: 'dark', language: 'en', notif_copy: 1, notif_new: 0 });
      }
      res.json(rows[0]);
    } catch (err: any) {
      res.status(500).json({ message: 'Error.', error: err.message });
  ...
  // PUT profile — upsert (insert or update)
  app.put('/api/profile', authenticateToken, async (req: any, res) => {
    const { display_name, email, theme, language, notif_copy, notif_new } = req.body;
    if (!display_name?.trim()) return res.status(400).json({ message: 'display_name is required.' });
    try {
      await db.execute({
        sql: `INSERT INTO user_profiles (user_id, display_name, email, theme, language, notif_copy, notif_new)
              VALUES (?, ?, ?, ?, ?, ?, ?)
              ON CONFLICT(user_id) DO UPDATE SET
                ...
                notif_copy   = excluded.notif_copy,
                notif_new    = excluded.notif_new`,
        args: [req.user.id, display_name, email ?? '', theme ?? 'dark', language ?? 'en', notif_copy ? 1 : 0, notif_new ? 1 : 0],
      });
