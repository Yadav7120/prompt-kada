        ...
        {prompts.map(p => (
          <div key={p.id} className="glass-card p-4 flex items-center gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-text-dim">{p.category_name} · {formatDate(p.created_at)} · {p.views} views</p>
            </div>
            <div className="flex items-center gap-2">
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="glass-card p-6 space-y-5">
        ...
        <div>
          ...
          <textarea
            className="input-field w-full min-h-48 resize-y"
            value={content}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="text-xs text-text-dim uppercase tracking-wider mb-2 block">Tags (comma separated)</label>
        ...
        <div className="flex gap-3">
          <button onClick={handleSave} disabled={saving} className="btn-primary">
            {saving ? 'Saving...' : <><Check size={18} /> {id ? 'Update Prompt' : 'Create Prompt'}</>}
          </button>
          <button onClick={() => navigate('/admin/manage')} className="btn-secondary">
            Cancel
          </button>
        </div>
      </div>
    ...
    <ProfileProvider>
      <ErrorBoundary>
        <Routes>
          ...
          <Route path="/login"          element={<div className="min-h-screen bg-bg-dark p-8"><AdminLoginPage /></div>} />
